...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.type === 'predict') {
      try {
        ...
        if (railwayResponse.ok) {
          ...
          if (railwayData && Object.keys(railwayData).length > 0) {
            return NextResponse.json(railwayData);
          }
        }
      } catch (railwayError: unknown) {
        if (railwayError instanceof Error) {
          console.log('Railway API failed, using fallback', railwayError.message);
        }
      }

      // Fallback to random data if Railway API fails or returns empty data
    ...
    // Alert endpoint
      ...
      // Fallback logic for alerts if API fails or returns empty data
      const now = new Date();
